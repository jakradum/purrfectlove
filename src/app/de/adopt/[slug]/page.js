...
export async function generateMetadata({ params }) {
  ...
  if (!cat) return {};
  const title = `${cat.name} – Katze adoptieren | Purrfect Love`;
  const description = cat.traits
    ? `${cat.traits} — ${cat.description || ''}`.trim().replace(/—\s*$/, '')
    : cat.description || `Lerne ${cat.name} kennen – eine Katze auf der Suche nach einem Zuhause.`;
  ...
  return {
    ...
    alternates: {
      canonical: `${BASE_URL}/de/adopt/${slug}`,
      languages: {
