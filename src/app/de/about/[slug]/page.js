...
export async function generateMetadata({ params }) {
  ...
  if (!member) {
    return {
      title: 'Teammitglied nicht gefunden - Purrfect Love',
    };
  }

  return {
    title: `${member.name} - Purrfect Love`,
    description: member.role ? `${member.name} - ${member.role} bei Purrfect Love` : `Lernen Sie ${member.name} vom Purrfect Love Team kennen`,
  };
}
...
export default async function TeamMember({ params }) {
  const { slug } = await params;
  return <TeamMemberPage slug={slug} locale="de" />;
}
