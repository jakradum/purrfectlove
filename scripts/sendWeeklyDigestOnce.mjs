// One-off script: sends the weekly digest to a custom address for preview.
// Usage:
//   node --env-file=.env.local scripts/sendWeeklyDigestOnce.mjs

import { createClient as createSanity } from '@sanity/client'
import { createClient as createSupabase } from '@supabase/supabase-js'
import { Resend } from 'resend'

const TO = 'pranavkarnad@gmail.com'
...
async function main() {
  ...
  console.log(`Querying data for ${dateRange}...`)

  // Membership requests
  const { data: membershipRequests = [], error: mrErr } = await supabase
    .from('membership_requests')
    .select('id, name, email, submitted_at, status')
    .gte('submitted_at', weekAgoIso)
    .order('submitted_at', { ascending: false })
  if (mrErr) console.error('membership_requests error:', mrErr)
  ...
  } catch (e) { console.error('contactMessage error:', e) }

  // Adoption applications
  let applications = []
  try {
    applications = await sanity.fetch(
  ...
  console.log(`Found: ${membershipRequests.length} membership requests, ${bookings.length} bookings, ${contactMessages.length} contact messages, ${applications.length} applications`)
  ...
  const intro = `
    ...
    <p style="font-size:12px;color:#aaa;margin:4px 0 0;">(Preview send to ${TO} — production digest goes to support@purrfectlove.org)</p>`

  const mrSection = sectionHeader('Membership Requests', membershipRequests.length) +
    table(
      ['Name', 'Email', 'Submitted', 'Status'],
      membershipRequests.map(r => [r.name, r.email, fmtDate(r.submitted_at), r.status])
    )
  ...
  const cmSection = sectionHeader('Contact Messages', contactMessages.length) +
    table(
      ...
      contactMessages.map(m => [
        m.name, m.email, fmtDate(m.submittedAt), m.status || '—',
        m.preview ? `${m.preview}${m.preview.length >= 90 ? '…' : ''}` : '—',
      ])
    )
  ...
  const apSection = sectionHeader('Adoption Applications', applications.length) +
    table(
      ['Name', 'Email', 'Submitted', 'Status'],
      applications.map(a => [a.applicantName, a.email, fmtDate(a._createdAt), a.status || '—'])
    )
  ...
  const { data, error } = await resend.emails.send({
    ...
    to: [TO],
    subject: `[Preview] Weekly Digest — ${dateRange}`,
